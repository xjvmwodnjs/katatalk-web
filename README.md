...
> **Repository status — 2026-08-19:** the current decision is **global public
...
> loss signals. Per-turn loss-perspective normalization and cross-axis tests,
> plus owner-scoped paid-submission idempotency, are now implemented locally;
> staged rollout and hosted evidence remain. Payment reversals, capacity
> admission, minimum-privilege storage/retention and
...
> [paid-analysis idempotency rollout](docs/analysis-idempotency-rollout-v1.md),
...
2026-08-19 NLC-005 구현: 브라우저는 계정-scope hash+SGF+언어 digest별 opaque
`Idempotency-Key`를 보존하고, 업로드 전에 owner-qualified recovery API로 이미
commit된 job을 찾는다. migration `015`의 v2 RPC는 같은 owner/key를
profile row lock 아래 직렬화한다. 동일 payload 재시도는 원래 job을 `200`
으로 반환하고 다른 payload는 `409`이며, 새 요청만 `202`와 1회 차감을
만든다. 상태/timeline 조회는 owner-qualified 경량 projection만 읽고,
완료 결과는 `/api/analyze/:jobId/result`에서 1회 가져오며 ETag와
`private, no-store`를 제공한다. route-level 100회 replay와 PostgreSQL
serial/32-way race·unique-index gate를 추가했다. 실제 Supabase 적용과 staged
Web/client rollout은 아직
필수이며, 공개 유료 traffic 전환은
[`analysis-idempotency-rollout-v1`](docs/analysis-idempotency-rollout-v1.md)의
POST 유지보수 차단·구 Web drain·required-header 순서를 따라야 한다. 외부 LLM
경로는 계속 OFF다.
...
의존성 override와 `wouter` patch는 root `pnpm-workspace.yaml`이 단일
설정원이며, `packageManager`는 SHA-512로 고정한 pnpm `10.18.1`이다. frozen
lockfile 검증과 production audit(현재 알려진 취약점 0)을 통과했으며 release/CI는
임의 전역 pnpm이 아니라 이 고정 버전을 사용한다.
...
- **P0 정확성 경계:** per-turn 후보는 이제 명시적 `lossPerspective`로 착수자 관점을 고정하고 BLACK/WHITE/SIDETOMOVE 교차축 회귀를 통과한다. 다만 Worker persistence, `scoreMean`/mixed metric, legacy partial metadata, 실제 engine corpus의 동일성 증거는 남아 있으므로 이 신호를 자연어 사실로 승격하지 않는다.
...
**최신 master(KataGo worker·lease·Deep Search v1) 배포 전**에는 [`docs/TODO.md`](docs/TODO.md) 의 **「최신 master 배포 전 smoke (체크리스트)」** 를 함께 수행하세요. 여기에는 **Supabase `001 → 015` 운영 DB 적용·권한 확인**, **Web/Worker env**, **Deep Search OFF 기본 검증**, **Deep Search ON 은 GPU worker 소규모만** 등이 정리되어 있습니다.
...
- [ ] Supabase migration **`001 → 015` 숫자 순서** 적용 및 DB gate 통과 (`012`: 원자 실패·환불, `013`: 기존 SECURITY DEFINER/ACL 고정, `014`: 격리 finalization 단일 작업 복구, `015`: 유료 분석 요청 멱등성)
...
1. Supabase 프로젝트에 migration **`001`→`015` 전체를 숫자순으로 적용**하고 migration/ACL gate를 통과해야 합니다. 일부 claim migration만 적용한 DB는 현재 계약이 아니며 worker·원장 RPC가 실패하거나 권한 경계가 달라질 수 있습니다.
...
- [`014_reconcile_analysis_job_finalization.sql`](supabase/migrations/014_reconcile_analysis_job_finalization.sql) — 격리된 누락 usage link를 검증·미리보기·단일 작업 확정으로만 복구하는 12번째 server-only RPC
- [`015_analysis_request_idempotency.sql`](supabase/migrations/015_analysis_request_idempotency.sql) — `(owner, request_id)` 멱등성, payload fingerprint 충돌 방지, rolling-deploy 호환 v2 enqueue RPC

파일을 골라 수동 실행하지 말고 반드시 `001 → 015` 숫자 순서를 사용한다. 새 runner와 실제 PostgreSQL gate는 [database migration gate](docs/database-migration-gate.md)를 따른다. 실제 staging 권한 증거는 SQL Editor 결과를 복사하는 대신 같은 커밋의 CI expected contract와 `pnpm db:evidence:staging` 읽기 전용 collector로 수집한다.
...
Supabase migration은 반드시 `001 → 002 → … → 015` 숫자순으로 적용합니다. `pnpm db:migrate:supabase`는 `PG*` 환경변수 또는 CI container를 사용하며, 이력 없는 기존 DB는 reviewed baseline이 없으면 fail-closed 됩니다. `pnpm test:db:migrations`는 명시적으로 확인한 폐기 가능 container에서만 fresh `001→015`, upgrade `011→012→013→014→015`, checksum drift, ACL/42501, rollback·quarantine·동시 lock wait 및 schema/ACL/함수 본문 구조 동등성을 검증하고 정제된 artifact를 보존합니다. `013`은 최초 11개 SECURITY DEFINER 함수의 owner/search_path=`pg_catalog`/ACL과 table ACL을 고정했고, `014`는 reconciliation RPC를 12번째 함수로, `015`는 멱등 enqueue v2를 13번째 함수로 추가합니다. 실제 staging은 master 전용 `Staging database evidence` workflow가 same-commit contract, clean checkout, DB↔HTTP Supabase project binding, read-only catalog와 PostgREST 거절을 검증한 뒤 정제된 evidence만 업로드합니다. 상세 설정과 기존 운영 DB 도입 제한은 [database-migration-gate](docs/database-migration-gate.md)를 참고하세요.
