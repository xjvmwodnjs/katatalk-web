...
1. `KATAGO_BINARY_PATH`가 CUDA/OpenCL/TensorRT KataGo를 가리키도록 로컬 env에 설정한다.
2. `KATAGO_CONFIG_PATH`가 analysis용 config를 가리키는지 확인한다.
3. `KATAGO_REQUIRE_GPU_BACKEND=true`를 설정한다.
4. Worker를 실행한다.
5. startup log에서 `katagoBackend=cuda`, `katagoBackend=opencl`, 또는 `katagoBackend=tensorrt`를 확인한다.
