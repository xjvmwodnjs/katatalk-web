...
- [~] COM-005 Japanese-only SGF rules admission and initial-player contract implemented: pre-mainline `PL[B|W]` priority, first-move fallback, matching `HA>=2`/final black setup count, and `HA[0]` exporter-compatible no-handicap are enforced; HA places no stones. Invalid/duplicate or move-node/post-move `PL`, invalid/duplicate `HA`, HA/setup count mismatch, out-of-board setup coordinates, and setup+move mixing return fixed non-reflective 400 before wallet/debit/enqueue. `initialPlayer` propagates through primary/spawn/persistent, multi-turn, deep search, benchmark, timeline, synthetic probe, and UI playback. Post-move transition, strict root `SZ`/`KM`, compressed setup ranges, full legality, additional rulesets, and PB/PW/DT/RE metadata remain required.
