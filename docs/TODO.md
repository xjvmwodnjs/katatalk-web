...
- [~] COM-005 Japanese-only SGF rules, strict root `SZ`/`KM`, and initial-player contracts implemented. Launch admission supports square 9/13/19 and integer/half-integer komi in [-150,150]; only missing values default to 19/6.5, with provenance retained in `analysis-plan-v1`. Explicit malformed/duplicate/multi-value/non-root mainline metadata, invalid/duplicate or move-node/post-move `PL`, invalid/duplicate `HA`, HA/setup mismatch, invalid coordinates, and setup+move mixing return fixed non-reflective 400 before wallet/debit/enqueue. Clerk upload auth now syncs identity without wallet provisioning; accepted uploads provision once after admission. Normalized board/komi and `initialPlayer` propagate through all KataGo query paths and UI playback uses root `SZ` only. Post-move transition, compressed setup ranges, full legality, additional rulesets, PB/PW/DT/RE metadata, real-exporter compatibility, and staging evidence remain required.
