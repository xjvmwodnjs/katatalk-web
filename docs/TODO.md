...
- [~] COM-005 Japanese-only SGF rules admission implemented: root `RU` is parsed structurally, reviewed Japanese aliases normalize to `japanese`, and unsupported rules return a fixed non-reflective 400 before wallet/debit/enqueue. `PL`, handicap/setup consistency, strict root `SZ`/`KM`, additional rulesets, and real game metadata remain required.
