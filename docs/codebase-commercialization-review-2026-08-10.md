...
| P1       | Broad global request parser                      | `server/_core/index.ts` installs 50 MB JSON and URL-encoded parsers globally although SGF upload has its own small limit.                          | Set endpoint-specific limits and add HTTP security headers/reverse-proxy limits before public traffic.                                                |
