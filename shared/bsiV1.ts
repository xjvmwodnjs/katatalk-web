...
/** best·played 동일 축으로 score 차이를 낼 때만 lead/mean; 축 불일치·미존재 시 none */
...
export type BsiV1SignalComponents = {
  ...
  /** `pickScoreDetail` 기준 best 행에 사용된 축 */
  bestScoreMetric?: BsiV1ScoreMetricUsed;
  /** `pickScoreDetail` 기준 played 행에 사용된 축 */
  playedScoreMetric?: BsiV1ScoreMetricUsed;
  /** best·played 가 서로 다른 score 축(lead vs mean 등)으로 비교 불가 */
...
export type BsiV1Signal = {
  ...
  playedMoveRank: number | null;
  /**
   * score 차이에 사용한 공통 축. best·played 가 **동일**하게 scoreLead(또는 둘 다 scoreMean)일 때만 lead/mean;
   * lead/mean 혼합·한쪽만 수치면 **none** (winrate·rank 블렌드만 가능).
   */
  scoreMetricUsed: BsiV1ScoreMetricUsed;
  ...
  /**
   * **동일 score metric** 일 때만: best − played 를 0 이상으로 clamp.
   * metric 혼합 시 **undefined**(잘못된 lead vs mean 비교 방지). 흑/백 손해로 단정하지 않음.
   */
  scoreBestMinusPlayed?: number;
  winrateBestMinusPlayed?: number;
  /** @deprecated 하위 호환 — `scoreBestMinusPlayed` 와 동일; mixed metric 시 생략 */
  scoreDelta?: number;
